...
export default function PremiumAdmin() {
  ...
  // STATE MỚI: Theo dõi nhóm nào đang được mở/gập
  ...
  // Hàm xử lý việc gập/mở khi click vào tên nhóm
            ...
            <tbody>
              {loading ? (
                ...
                Object.entries(groupedLinks).map(([netName, group]) => {
                  // KIỂM TRA TRẠNG THÁI GẬP MỞ (Sẽ tự mở nếu đang tìm kiếm, ngược lại check state)
                  const isExpanded = search !== '' || expandedGroups[netName];

                  return (
                    <React.Fragment key={netName}>
                      {/* DÒNG TIÊU ĐỀ BẤM ĐƯỢC (CLICKABLE HEADER) */}
                      <tr 
                        onClick={() => toggleGroup(netName)}
                        style={{ background: '#1e293b', borderBottom: '1px solid #334155', cursor: 'pointer', transition: 'background 0.2s', userSelect: 'none' }}
                      ...
                      >
                        <td colSpan="4" style={{ padding: '12px 24px', fontWeight: '700', color: group.info.text }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            {/* Mũi tên chỉ xuống hoặc xoay ngược khi mở */}
                            <svg 
                      ...
                      {/* CHỈ RENDER LINK NẾU isExpanded = TRUE */}
                        ...
                        <tr key={l.id} style={{ borderBottom: '1px solid #1f2937', transition: 'background 0.15s', animation: 'fadeIn 0.2s ease-out' }} onMouseEnter={(e) => e.currentTarget.style.background = '#181b23'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                          ...
                          <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                              <button onClick={() => handleCopy(l.slug)} title="Copy" style={{ background: '#374151', color: '#d1d5db', border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center' }} onMouseEnter={(e) => { e.currentTarget.style.background = '#4b5563'; e.currentTarget.style.color = '#fff'; }} onMouseLeave={(e) => { e.currentTarget.style.background = '#374151'; e.currentTarget.style.color = '#d1d5db'; }}>
                              ...
                              <a href={l.original_url} target="_blank" rel="noopener noreferrer" title="Mở Link Gốc" style={{ background: '#374151', color: '#d1d5db', border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', textDecoration: 'none' }} onMouseEnter={(e) => { e.currentTarget.style.background = '#4b5563'; e.currentTarget.style.color = '#fff'; }} onMouseLeave={(e) => { e.currentTarget.style.background = '#374151'; e.currentTarget.style.color = '#d1d5db'; }}>
                              ...
                              <button onClick={() => handleDelete(l.slug)} title="Xóa" style={{ background: '#374151', color: '#fca5a5', border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center' }} onMouseEnter={(e) => { e.currentTarget.style.background = '#ef4444'; e.currentTarget.style.color = '#fff'; }} onMouseLeave={(e) => { e.currentTarget.style.background = '#374151'; e.currentTarget.style.color = '#fca5a5'; }}>
