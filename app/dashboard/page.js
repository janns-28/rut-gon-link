...
// Thay link edit sheet của mày vào đây
...
export default function Dashboard() {
  ...
  // Hàm helper để bỏ http:// cho gọn khi hiển thị
  const formatShortLink = (slug) => {
    if (!origin) return `/${slug}`;
    const domain = origin.replace(/(^\w+:|^)\/\//, ''); // Bỏ http:// hoặc https://
    return `${domain}/${slug}`;
  };
  ...
  return (
    <>
      <style jsx global>{`
        body { background-color: #030303; color: #fff; font-family: 'Inter', -apple-system, sans-serif; }
      ...
      {/* --- SCROLL WRAPPER --- */}
      <div className="scroll-container" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100vh', overflowY: 'auto', overflowX: 'hidden' }}>
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 20px', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
          ...
          <section className="glass-box" style={{ padding: '30px', marginBottom: '50px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '15px' }}>
              <div style={{ position: 'relative' }}>
                <input className="input-field" type="text" placeholder="Dán link dài ngoằng vào đây..." value={newUrl} onChange={(e) => setNewUrl(e.target.value)} style={{ width: '100%', padding: '16px 20px', borderRadius: '16px', fontSize: '1rem', boxSizing: 'border-box' }} />
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <input className="input-field" type="text" placeholder="Slug" value={newSlug} onChange={(e) => setNewSlug(e.target.value)} style={{ width: '100%', padding: '16px', borderRadius: '16px', fontSize: '1rem', fontWeight: 'bold', color: '#ff0080', textAlign: 'center', boxSizing: 'border-box' }} />
                <button onClick={generateRandomSlug} style={{ width: '50px', background: '#222', border: '1px solid #333', borderRadius: '16px', color: '#888', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Random">🎲</button>
              </div>
              <button onClick={handleCopyDataToSheet} className="primary-btn" style={{ borderRadius: '16px', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>+ Tạo ngay</button>
            </div>
          </section>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <input className="input-field" type="text" placeholder="🔍 Tìm nhanh..." value={filter} onChange={(e) => setFilter(e.target.value)} style={{ width: '250px', padding: '10px 15px', borderRadius: '12px', fontSize: '0.9rem' }} />
            </div>
                ...
                filteredData.map((row, index) => (
                  <div key={index} className="glass-box link-card" style={{ padding: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px', overflow: 'hidden' }}>
                      <div style={{ width: '50px', height: '50px', borderRadius: '14px', background: 'linear-gradient(135deg, #222, #111)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a855f7', fontWeight: 'bold', fontSize: '1.2rem', border: '1px solid #333', flexShrink: 0 }}>
                        {row[0].charAt(0).toUpperCase()}
                      </div>

                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', overflow: 'hidden' }}>
                        {/* --- PHẦN SỬA Ở ĐÂY: HIỆN FULL LINK --- */}
                    ...
                    <div style={{ display: 'flex', gap: '10px', flexShrink: 0 }}>
                      <button onClick={() => copyToClipboard(`${origin}/${row[0]}`, row[0])} style={{ background: copied === row[0] ? '#00FF94' : '#222', color: copied === row[0] ? '#000' : '#fff', border: '1px solid #333', width: '45px', height: '45px', borderRadius: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: '0.2s' }} title="Copy Link">
                        {copied === row[0] ? <CheckIcon /> : <CopyIcon />}
                      </button>
                      <a href={`${origin}/${row[0]}`} target="_blank" style={{ background: '#222', color: '#fff', border: '1px solid #333', width: '45px', height: '45px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', transition: '0.2s' }} title="Mở link">↗</a>
                    </div>
                  </div>
                ))
              )}
