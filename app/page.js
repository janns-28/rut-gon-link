...
const HOLIDAYS = {
  '01-01': 'Tết Dương Lịch', '30-04': 'Giải Phóng Miền Nam',
  '01-05': 'Quốc Tế Lao Động', '02-09': 'Quốc Khánh Việt Nam',
  '24-12': 'Giáng Sinh', '31-12': 'Giao Thừa'
};

export default function AutoQuoteClock() {
  const [mounted, setMounted] = useState(false);
  const [now, setNow] = useState(new Date());
  const [dynamicQuote, setDynamicQuote] = useState('Đang lấy câu khịa từ vũ trụ...');

  useEffect(() => {
    setMounted(true);
    
    // TỰ ĐỘNG LÊN MẠNG LẤY CÂU KHÌA
    const fetchQuote = async () => {
      try {
        const res = await fetch('/api/random-quote');
        const data = await res.json();
        setDynamicQuote(data.quote);
      } catch (e) {
    ...
    <div style={st.wrap}>
      <div style={st.container}>
        ...
        <div style={st.countdownArea}>
          <p style={st.countLabel}>HÀNH TRÌNH ĐẾN NĂM MỚI {now.getFullYear() + 1}</p>
          <div style={st.grid}>
